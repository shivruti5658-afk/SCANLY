...
import {Camera,FileText,Download,Plus,ScanLine,X,Image as ImageIcon,Check,Crop,ChevronLeft,ChevronRight,RefreshCw,Flashlight,FlashlightOff,Sparkles,Clock,Pencil,Trash2,ShieldCheck,RotateCcw,RotateCw,Type} from 'lucide-react';
import './style.css';
...
type Page={id:number;src:string;filter:Filter;crop:CropBox;corners?:Corners};
...
function App(){
 ...
 useEffect(()=>{
  try{const raw=localStorage.getItem(HISTORY_KEY);if(raw)setRecent(JSON.parse(raw));}catch{}
 ...
 async function rotatePage(id:number,direction:1|-1){
  ...
  const img=new Image();await new Promise<void>(resolve=>{img.onload=()=>resolve();img.src=page.src});
  const c=document.createElement('canvas');c.width=img.height;c.height=img.width;
  const ctx=c.getContext('2d')!;
  if(direction===1){ctx.translate(c.width,0);ctx.rotate(Math.PI/2)}
  else{ctx.translate(0,c.height);ctx.rotate(-Math.PI/2)}
  ctx.drawImage(img,0,0);
  update(id,{src:c.toDataURL('image/jpeg',.98),crop:{...fullCrop},corners:undefined});
 }
 ...
 function beginCorner(key:CornerKey,e:React.PointerEvent){
  e.preventDefault();e.stopPropagation();cornerDrag.current={key};
  (e.currentTarget as HTMLElement).setPointerCapture?.(e.pointerId);
 }
 ...
 function moveCorner(e:React.PointerEvent){
  ...
  const base=current.corners||{
   tl:{x:current.crop.x,y:current.crop.y},tr:{x:current.crop.x+current.crop.w,y:current.crop.y},
   br:{x:current.crop.x+current.crop.w,y:current.crop.y+current.crop.h},bl:{x:current.crop.x,y:current.crop.y+current.crop.h}
  };
  const corners={...base,[cornerDrag.current.key]:{x,y}};
  update(current.id,{corners});
 }
 const endCrop=()=>{drag.current=null;cornerDrag.current=null};

 async function compactForHistory(src:string){
  const img=new Image();
  await new Promise<void>(resolve=>{img.onload=()=>resolve();img.src=src});
  const maxSide=1100,scale=Math.min(1,maxSide/Math.max(img.width,img.height));
 ...
 async function saveToHistory(){
  ...
  try{
   const savedPages=await Promise.all(pages.map(async p=>({...p,src:await compactForHistory(p.src)})));
   const item:SavedScan={id:`${Date.now()}-${Math.random()}`,name:name.trim()||'Scanned Document',pages:savedPages,createdAt:Date.now()};
   const next=[item,...recent].slice(0,MAX_HISTORY);
   localStorage.setItem(HISTORY_KEY,JSON.stringify(next));setRecent(next);
  }catch{alert('This scan is too large to keep in local storage. Try fewer pages or smaller images.')}
 }
 function openSaved(item:SavedScan){
  stopCamera();setPages(item.pages.map(p=>({...p,id:Date.now()+Math.random()})));setName(item.name);setSelected(null);setEditing(true);
 }
 function deleteSaved(id:string){
  const next=recent.filter(x=>x.id!==id);setRecent(next);
 ...
 async function makePdf(){
  ...
  for(const p of pages){
   ...
   const points=p.corners?Object.values(p.corners):[{x:p.crop.x,y:p.crop.y},{x:p.crop.x+p.crop.w,y:p.crop.y},{x:p.crop.x+p.crop.w,y:p.crop.y+p.crop.h},{x:p.crop.x,y:p.crop.y+p.crop.h}];
   const minX=Math.min(...points.map(q=>q.x)),maxX=Math.max(...points.map(q=>q.x));
   const minY=Math.min(...points.map(q=>q.y)),maxY=Math.max(...points.map(q=>q.y));
   const sx=img.width*minX/100,sy=img.height*minY/100,sw=img.width*(maxX-minX)/100,sh=img.height*(maxY-minY)/100;
   ...
   if(p.corners){
    ...
    points.forEach((q,i)=>{const x=(q.x-minX)/(maxX-minX)*canvas.width,y=(q.y-minY)/(maxY-minY)*canvas.height;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.closePath();ctx.clip();
   }
 ...
 {!scanning&&!editing&&<section className="home-screen">
  <div className="premium-hero">
   <div className="hero-copy">
    <div className="hero-kicker"><Sparkles size={15}/> PREMIUM DOCUMENT SCANNER</div>
    <div className="logo-lockup"><div className="logo-orb"><ScanLine size={34}/></div><div><h1>Scan beautifully.<br/><em>Keep everything editable.</em></h1></div></div>
    <p>Capture documents, enhance clarity, fine-tune the crop, and turn multiple pages into a clean PDF — directly on your device.</p>
    <div className="actions"><button className="primary home-primary" onClick={openCamera}><Camera size={20}/> Start scanning</button><label className="secondary home-secondary"><ImageIcon size={19}/> Import images<input type="file" accept="image/*" multiple onChange={upload}/></label></div>
    <div className="trust-row"><span><ShieldCheck size={16}/> Local-first</span><span><Sparkles size={16}/> Smart enhancement</span><span><FileText size={16}/> Multi-page PDF</span></div>
   </div>
   <div className="scan-showcase"><div className="showcase-glow"></div><div className="floating-doc doc-back"></div><div className="floating-doc doc-front"><div className="mini-logo"><ScanLine size={18}/> Scanly</div><div className="doc-lines"><i></i><i></i><i></i><i></i></div><div className="doc-stamp">READY</div></div><div className="showcase-chip"><Sparkles size={15}/> Clear. Fast. Private.</div></div>
  </div>
  <section className="recent-section">
   <div className="section-title"><div><span className="section-eyebrow"><Clock size={14}/> YOUR DEVICE</span><h2>Recent editable scans</h2></div><span className="recent-count">{recent.length}/5 saved</span></div>
   {recent.length?<div className="recent-grid">{recent.map(item=><article className="recent-card" key={item.id}><button className="recent-main" onClick={()=>openSaved(item)}><div className="recent-thumb">{item.pages[0]&&<img src={item.pages[0].src}/>}<span>{item.pages.length} pg</span></div><div className="recent-info"><strong>{item.name}</strong><small>{new Date(item.createdAt).toLocaleDateString()} · Editable</small></div><Pencil size={17}/></button><button className="recent-delete" onClick={()=>deleteSaved(item.id)} aria-label={`Delete ${item.name}`}><Trash2 size={15}/></button></article>)}</div>:<div className="recent-empty"><div><Clock size={23}/></div><strong>Your last 5 scans will appear here</strong><span>Download a PDF once, then reopen it later to continue editing.</span></div>}
  </section>
 </section>}
 ...
 {editing&&!scanning&&current&&<section className="editor">
  ...
  <div className="simple-editor">
   ...
   <div className="preview-area"><div ref={cropArea} className="crop-stage" onPointerMove={e=>{moveCrop(e);moveCorner(e)}} onPointerUp={endCrop} onPointerCancel={endCrop}>
    <img className="stationary-image" src={current.src} style={{filter:filterStyle(current.filter)}}/>
    <div className="crop-mask" style={{left:`${current.crop.x}%`,top:`${current.crop.y}%`,width:`${current.crop.w}%`,height:`${current.crop.h}%`}}><div className="crop-box">{(['t','b','l','r'] as Handle[]).map(h=><span key={h} className={'handle '+h} onPointerDown={e=>beginCrop(h,e)}/>)}</div></div>
    <svg className="corner-crop" viewBox="0 0 100 100" preserveAspectRatio="none">
     {(()=>{const c=current.corners||{tl:{x:current.crop.x,y:current.crop.y},tr:{x:current.crop.x+current.crop.w,y:current.crop.y},br:{x:current.crop.x+current.crop.w,y:current.crop.y+current.crop.h},bl:{x:current.crop.x,y:current.crop.y+current.crop.h}};return <><polygon points={`${c.tl.x},${c.tl.y} ${c.tr.x},${c.tr.y} ${c.br.x},${c.br.y} ${c.bl.x},${c.bl.y}`} className="corner-shape"/>{(['tl','tr','br','bl'] as CornerKey[]).map(k=><circle key={k} cx={c[k].x} cy={c[k].y} r="1.9" className="corner-handle" onPointerDown={e=>beginCorner(k,e)}/>)}</>})()}
    </svg>
   </div></div>
   <div className="edit-controls"><div className="crop-label"><Crop size={17}/> Drag corners or edges to crop</div><div className="image-tools"><button className="reset-crop" onClick={()=>rotatePage(current.id,-1)}><RotateCcw size={16}/> Rotate left</button><button className="reset-crop" onClick={()=>rotatePage(current.id,1)}><RotateCw size={16}/> Rotate right</button><button className="reset-crop" onClick={()=>update(current.id,{crop:{...fullCrop},corners:undefined})}><RefreshCw size={16}/> Reset crop</button></div><div className="filter-buttons">{filters.map(f=><button key={f} className={current.filter===f?'chosen':''} onClick={()=>update(current.id,{filter:f})}>{f}</button>)}</div></div>
  </div>
  <div className="export"><div className="filename-box"><label><Type size={14}/> PDF filename <span>changes instantly</span></label><input value={name} placeholder="Enter PDF name" onChange={e=>setName(e.target.value)} onFocus={e=>e.currentTarget.select()}/><small>{name.trim()||'Scanned Document'}.pdf</small></div><button className="primary" onClick={makePdf}><Download size={19}/> Download PDF</button></div>
 </section>}
