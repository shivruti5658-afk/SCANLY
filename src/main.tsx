...
function filteredCanvas(p: Page): HTMLCanvasElement {
  ...
  const sw = (img.width * p.crop.w) / 100,
...
function App() {
  ...
  async function openCamera() {
    ...
    try {
      ...
      if (settings?.width && settings?.height)
        setCameraInfo(`${settings.width} × ${settings.height}`);
      if (video.current) video.current.srcObject = stream.current;
    } catch {
      alert(
        "Camera access was denied or is unavailable. You can still add images.",
      );
      setScanning(false);
    }
  }
  ...
  function capture() {
    ...
    c.toBlob(
      (blob) => {
        if (blob) addPage(URL.createObjectURL(blob));
      },
      "image/jpeg",
      0.98,
    );
  ...
  function setCropEdge(
    ...
    const right = 100 - x - w,
      bottom = 100 - y - h;
    if (edge === "left")
      update(current.id, { crop: { x: value, y, w: 100 - value - right, h } });
    if (edge === "right")
      update(current.id, { crop: { x, y, w: 100 - x - value, h } });
    if (edge === "top")
      update(current.id, { crop: { x, y: value, w, h: 100 - value - bottom } });
    if (edge === "bottom")
      update(current.id, { crop: { x, y, w, h: 100 - y - value } });
  }
  async function makePdf() {
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      const data = temp.toDataURL("image/jpeg", 0.98);
      const ratio = sw / sh,
        maxW = 194,
        maxH = 281;
      let w = maxW,
          ...
          <>
            <section className="hero">
              <div>
                ...
                <p className="sub">
                  Keep the camera open and scan multiple documents one after
                  another. When you are done, crop and apply filters to every
                  page before creating one PDF.
                </p>
              ...
              <div className="hero-card">
                ...
                <span>Scan many pages, then edit them together.</span>
              </div>
            </section>
          </>
        )}
        {editing && !scanning && current && (
          <section className="editor">
            <div className="editor-head">
              <div>
                <p className="eyebrow">EDIT SCANNED DOCUMENTS</p>
                <h2>
                      ...
                      filter:
                        current.filter === "Grayscale"
                          ...
                          : current.filter === "B&W"
                            ...
                            : "none",
                      clipPath: `inset(${current.crop.y}% ${100 - current.crop.x - current.crop.w}% ${100 - current.crop.y - current.crop.h}% ${current.crop.x}%)`,
                      transform: `rotate(${current.rotation}deg)`,
                    }}
                  />
                </div>
                <span className="crop-note">
                  Preview reflects crop and filter
                </span>
              </div>
              <aside className="tools">
                <div className="tool-title">
                  <Crop size={18} /> Crop all sides
                </div>
                <label>
                  Left <b>{current.crop.x}%</b>
                  <input
                    type="range"
                    min="0"
                    max={
                      100 -
                      MIN_CROP_SIZE -
                      (100 - current.crop.x - current.crop.w)
                    }
                    value={current.crop.x}
                    onChange={(e) => setCropEdge("left", +e.target.value)}
                  />
                </label>
                <label>
                  Right <b>{100 - current.crop.x - current.crop.w}%</b>
                  <input
                    type="range"
                    min="0"
                    max={100 - MIN_CROP_SIZE - current.crop.x}
                    value={100 - current.crop.x - current.crop.w}
                    onChange={(e) => setCropEdge("right", +e.target.value)}
                  />
                </label>
                <label>
                  Top <b>{current.crop.y}%</b>
                  <input
                    type="range"
                    min="0"
                    max={
                      100 -
                      MIN_CROP_SIZE -
                      (100 - current.crop.y - current.crop.h)
                    }
                    value={current.crop.y}
                    onChange={(e) => setCropEdge("top", +e.target.value)}
                  />
                </label>
                <label>
                  Bottom <b>{100 - current.crop.y - current.crop.h}%</b>
                  <input
                    type="range"
                    min="0"
                    max={100 - MIN_CROP_SIZE - current.crop.y}
                    value={100 - current.crop.y - current.crop.h}
                    onChange={(e) => setCropEdge("bottom", +e.target.value)}
                  />
                </label>
                <button
                  className="reset-crop"
                  onClick={() =>
                    update(current.id, { crop: { ...defaultCrop } })
                  }
                >
                  <SlidersHorizontal size={16} /> Reset crop
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    setPages((ps) => ps.filter((x) => x.id !== current.id));
                    setSelected(
                      pages.find((x) => x.id !== current.id)?.id ?? null,
                    );
                  }}
                >
        ...
        {scanning && (
          <section className="camera">
            ...
            <div className="capture-bottom">
              ...
              <p>Capture each document. The camera stays open.</p>
            </div>
          </section>
        )}
      </main>
